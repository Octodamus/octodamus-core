...
sys.path.insert(0, "/home/walli/octodamus")
...
# NOTE: No bitwarden import here — bot uses env vars set by run_octo_boto.sh

# ─── Logging ──────────────────────────────────────────────────────────────────
Path("/home/walli/octodamus/logs").mkdir(parents=True, exist_ok=True)
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[
        logging.StreamHandler(),
        logging.FileHandler("/home/walli/octodamus/logs/octo_boto.log"),
    ],
)
log = logging.getLogger("OctoBoto")

# ─── Config ───────────────────────────────────────────────────────────────────
AUTO_SCAN_INTERVAL = 30 * 60    # 30 minutes between auto scans
SCAN_LIMIT         = 150        # Markets to fetch per scan (paginated)
AI_BATCH_LIMIT     = 15         # Max markets sent to AI per scan
AUTO_MIN_EV        = 0.07       # Stricter 7% threshold in auto mode
AUTO_MAX_ENTER     = 3          # Max new positions per auto-scan
SCAN_COOLDOWN      = 3 * 60     # Prevent /scan spam — 3 min cooldown
MIN_CONF_AUTO      = {"high", "medium"}   # confidence levels that trigger auto-entry
...
def load_secrets() -> dict:
    """Load bot secrets from environment variables only.
    Keys are set by run_octo_boto.sh via bw get before launch."""
    secrets = {}
    for key in ("OCTOBOTO_TELEGRAM_TOKEN", "ANTHROPIC_API_KEY"):
        val = os.getenv(key)
        if val:
            secrets[key] = val
            log.info(f"[Secrets] {key} loaded from environment")
        else:
            log.warning(f"[Secrets] {key} not set in environment")
    return secrets
...
async def cmd_newwallet(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if "note" in wallet:
        await update.message.reply_text(
            f"⚠️ {wallet['note']}\n\nRun in WSL:\n`pip install eth-account web3 --break-system-packages`",
            parse_mode=ParseMode.MARKDOWN
        )
        return
