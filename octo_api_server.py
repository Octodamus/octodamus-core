"""
octo_api_server.py — Octodamus API Server v2
...
Endpoints:
  Public:
    ...
    GET /api/report              Live HTML report viewer (ACP link)
...
def validate_key(api_key: str) -> Optional[dict]:
    ...
    if entry.get("expires"):
        if datetime.fromisoformat(entry["expires"]) < datetime.utcnow(): return None
    return entry
...
def load_snapshot(snapshot_type: str, target_date: Optional[str] = None) -> dict:
    if target_date:
        try: d = date.fromisoformat(target_date)
        except ValueError: raise HTTPException(status_code=400, detail="Use YYYY-MM-DD")
        path = DATA_DIR / str(d) / f"{snapshot_type}.json"
        if not path.exists(): raise HTTPException(status_code=404, detail=f"No {snapshot_type} snapshot for {d}")
...
def acp_report(
    ...
    timeframe: str = Query("4h", description="Chart timeframe for analysis"),
):
    """
    Live HTML report — full styled report page.
    Linked from every ACP deliverable so buyers can view formatted report.
    No API key required.
    """
    try:
        import sys; sys.path.insert(0, str(Path(__file__).parent))
        ...
        # Load embedded logo if available
        logo_uri = ""
        ...
        error_html = f"""<!DOCTYPE html><html><body style="background:#03080f;color:#ff4d6d;font-family:monospace;padding:40px">
<h2>Octodamus Report Error</h2><pre>{e}</pre></body></html>"""
